...
- **Estado actual:** `ENTRY-ONB-005 — MIGRATIONS APPLIED TO HOSTED DEV; RUNTIME TESTS PENDING`.
...
- `ENTRY-ONB-002`: applied to hosted dev; runtime tests pending.
- `ENTRY-ONB-003`: applied to hosted dev; runtime tests pending.
- `ENTRY-ONB-004-UNBLOCK`: completed.
- `ENTRY-ONB-004`: completed; baseline `ac25878`.
- `ENTRY-ONB-005`: hosted dev migrations applied; runtime tests pending.
...
- El bloqueo de transporte seguro queda resuelto; runtime tests siguen pendientes.
- Sin push/PR/deployment.
- Backend de conversion implementado, auditado y commiteado en baseline `ac25878`.
- Estado vigente: `ENTRY-ONB-005 — MIGRATIONS APPLIED TO HOSTED DEV; RUNTIME TESTS PENDING`.
