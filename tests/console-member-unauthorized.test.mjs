...
test("unauthorized page is role-aware and does not redirect builders/viewers to dashboard", () => {
  ...
  assert.match(page, /isOwner = context\.status === "authorized" && context\.role === "owner"/);
  assert.match(page, /isMember = context\.status === "authorized" && context\.role !== "owner"/);
  ...
  assert.doesNotMatch(page, /if \(context\.status === "authorized" && !isSignOutError\)/);
  assert.match(page, /This area requires Owner access/);
  assert.match(page, /Your Minerva Console account is active, but this area is restricted to Owners/);
});
