-- Hotfix: the duplicate merge RPC inserted a new active Submitted version
-- before superseding the current active submission. The partial unique index
...
comment on function public.merge_community_registration_units_v1(
  ...
  'ENTRY internal RPC. Merges two safe Submitted registration duplicates. Supersedes the canonical active submission before inserting its merged replacement so idx_cr_submissions_one_active_per_unit remains satisfied; all changes remain transactional.';
