...
begin
  ...
  -- Email remains the preferred activation method whenever it is available.
  ...
  -- A PIN delivered to an old phone must not remain usable. Email-based
  -- activation is preserved because changing the secondary phone does not
  -- change the activation identity or the email recipient.
  v_requires_reset :=
    v_queue.activation_method = 'phone_pin'
    or v_new_method = 'phone_pin';
...
comment on function public.update_resident_activation_phone_v1(uuid, uuid, text) is
  'Superadmin-only pre-activation phone correction. Preserves email activation state, '
