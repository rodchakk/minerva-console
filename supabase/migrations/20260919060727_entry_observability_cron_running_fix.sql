...
begin
  ...
  select
    'resident_access:resident_house_context_invalid',rc.community_id::text,rc.community_id,
    'resident_access','RESIDENT_HOUSE_CONTEXT_INVALID','RESIDENT_HOUSE_CONTEXT_INVALID',
    case when count(*) filter(where not rc.valid)=count(*) then 'CRITICAL' else 'ERROR' end,
    jsonb_build_object('active_residents',count(*),'invalid_residents',count(*) filter(where not rc.valid))
  ...
  select
    ...
    'COMMUNICATIONS_WORKER_UNHEALTHY',coalesce(h.last_error_code,'COMMUNICATIONS_WORKER_STALE'),
    'ERROR',
  ...
  select
    ...
    jsonb_build_object('active_guards',count(distinct cm.user_id),'push_ready_guards',count(distinct upt.user_id))
  ...
  return jsonb_build_object(
    ...
    'upserted',v_upserted,'resolved',v_resolved,'checked_at',now()
  );
end;
$$;

revoke all on function public.reconcile_entry_observability_incidents_v1()
  from public,anon,authenticated;
grant execute on function public.reconcile_entry_observability_incidents_v1()
  to service_role;
