...
function MiniMetric({ badge, hint, label, value }: { badge?: string; hint: string; label: string; value: string | number }) {
  return (
...
function SummaryCard({ action, children, title }: { action?: ReactNode; children: ReactNode; title: string }) {
  return (
    <section className="rounded-[28px] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[0_16px_42px_rgba(2,6,23,0.2)]">
      <div className="flex items-start justify-between gap-4">
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-violet-200">{title}</p>
        {action}
      </div>
...
function QuickActionCard({ action }: { action: ActionItem }) {
  return (
    <Link href={action.href} className="group flex items-center justify-between gap-3 rounded-[22px] border border-white/8 bg-white/4 p-4 transition hover:border-violet-300/40 hover:bg-white/7">
      <div>
        <p className="text-sm font-semibold text-white">{action.label}</p>
        <p className="mt-1 text-sm leading-6 text-[var(--text-muted)]">{action.note}</p>
      </div>
      <span className="text-lg text-[var(--text-muted)] transition group-hover:translate-x-0.5 group-hover:text-white">→</span>
    </Link>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Badge tone={community.isActive ? "success" : "default"}>{community.isActive ? "Active" : "Inactive"}</Badge>
      ...
      <section className="rounded-[32px] border border-[var(--border)] bg-[linear-gradient(135deg,rgba(112,104,255,0.16),rgba(17,24,39,0.88)_42%,rgba(8,11,22,0.96))] p-5 shadow-[0_24px_70px_rgba(2,6,23,0.28)] backdrop-blur xl:p-6">
        <p className="text-xs font-semibold uppercase tracking-[0.26em] text-violet-200">Operational health</p>
        <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_1.25fr_1fr_1.25fr] xl:divide-x xl:divide-white/8">
          <div className="flex gap-4 xl:pr-5">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--text-muted)]">Current status</p>
              <h2 className="mt-2 text-xl font-semibold text-white">{getSetupLabel(community)}</h2>
              <p className="mt-1 text-sm leading-6 text-[var(--text-muted)]">
                {getSetupLabel(community) === "Complete" ? "Community is ready for regular operation." : "Action required to reach full readiness."}
              </p>
            </div>
          </div>

          <div className="flex items-center gap-5 xl:px-5">
            <div className="grid h-20 w-20 shrink-0 place-items-center rounded-full p-1" style={{ background: `conic-gradient(var(--primary) ${progressPercent}%, rgba(255,255,255,0.12) 0)` }}>
              <div className="grid h-full w-full place-items-center rounded-full bg-[var(--surface-strong)] text-sm font-semibold text-white">{progressPercent}%</div>
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--text-muted)]">Setup progress</p>
              <p className="mt-2 text-xl font-semibold text-white">{community.completedTasks} / {community.totalTasks} tasks complete</p>
              <div className="mt-3 h-2 rounded-full bg-white/8"><div className="h-2 rounded-full bg-[var(--primary)]" style={{ width: getProgressWidth(community.completedTasks, community.totalTasks) }} /></div>
            </div>
          </div>
          ...
          <div className="xl:pl-5">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--text-muted)]">What needs attention</p>
            ...
            <a href="#setup-status" className="mt-3 inline-flex text-sm font-semibold text-violet-200 transition hover:text-white">View setup details →</a>
          </div>
        </div>
      </section>
      ...
      <section className="grid overflow-hidden rounded-[28px] border border-[var(--border)] bg-[var(--surface)] shadow-[0_18px_48px_rgba(2,6,23,0.2)] md:grid-cols-2 xl:grid-cols-5">
        ...
        <MiniMetric badge="▤" label="Facilities" value={previews.facilities.state === "live" ? previews.facilities.activeCount : "—"} hint={previews.facilities.state === "live" ? "Active facilities" : previews.facilities.state === "disabled" ? "Reservations disabled" : "Not configured"} />
        <MiniMetric badge="⌁" label="Admin activity" value={adminActivity.state === "live" ? adminActivity.total : "—"} hint={getPreviewMetricStatus(adminActivity.state)} />
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-6">
          <section id="units-snapshot" className="rounded-[30px] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[0_18px_50px_rgba(2,6,23,0.2)] xl:p-6">
            ...
            {previews.units.state === "unavailable" ? (
              ...
              <div className="mt-5"><EmptyInline>No units created yet.</EmptyInline></div>
            ) : (
              <div className="mt-5 overflow-x-auto">
                <table className="min-w-[820px] w-full text-left text-sm">
                  <thead className="border-b border-white/8 text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">
                    <tr><th className="py-3 pr-4 font-semibold">Unit / building</th><th className="px-4 py-3 font-semibold">Owner</th><th className="px-4 py-3 font-semibold">Active residents</th><th className="px-4 py-3 font-semibold">Passes</th><th className="px-4 py-3 font-semibold">Last access</th><th className="py-3 pl-4 text-right font-semibold">Status</th></tr>
                  </thead>
                  <tbody className="divide-y divide-white/7">
                    {unitsForSnapshot.map((unit, index) => (
                      <tr key={unit.id} className="text-[var(--text-muted)]">
                        <td className="py-4 pr-4"><div className="flex items-center gap-3"><span className="grid h-8 w-8 place-items-center rounded-full border border-white/10 bg-white/5 text-xs font-semibold text-white">{String(index + 1).padStart(2, "0")}</span><span className="font-semibold text-white">{unit.label}</span></div></td>
                        <td className="px-4 py-4">{unit.ownerName}</td><td className="px-4 py-4 font-semibold text-white">{unit.activeResidents}</td><td className="px-4 py-4 font-semibold text-white">{unit.activePasses}</td><td className="px-4 py-4 text-white">{unit.lastAccess}</td><td className="py-4 pl-4 text-right"><Badge tone={unit.isActive ? "success" : "default"}>{unit.isActive ? "Active" : "Inactive"}</Badge></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <SummaryCard title="Users summary" action={<CommunityUsersDrawer communityId={community.id} users={previews.users.items} state={previews.users.state} triggerLabel="Review users" />}>
              <div id="users-summary" className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
                {[["Admins", previews.users.counts.admins], ["Guards", previews.users.counts.guards], ["Residents", previews.users.counts.residents], ["Inactive", previews.users.counts.inactive]].map(([label, value]) => (
                  <div key={label} className="border-r border-white/8 last:border-r-0"><p className="text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">{label}</p><p className="mt-2 text-2xl font-semibold text-white">{value}</p></div>
                ))}
              </div>
              {previews.users.state === "live" ? (
                <div className="mt-5 rounded-[22px] border border-white/8 bg-white/4 px-4 py-4">
                  <div className="flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--text-muted)]">Community operators</p><span className="text-xs text-[var(--text-muted)]">Admins and guards</span></div>
                  {communityOperators.length > 0 ? <div className="mt-4 space-y-3">{communityOperators.slice(0, 4).map((user) => <div key={user.id} className="flex items-center justify-between gap-3 rounded-2xl border border-white/8 bg-[var(--surface-strong)] px-3 py-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-white">{user.fullName}</p><p className="mt-1 text-sm text-[var(--text-muted)]">{user.contact}</p></div><div className="shrink-0 text-right"><Badge tone="info">{user.role}</Badge><p className="mt-1 text-xs text-[var(--text-muted)]">{user.houseLabel}</p></div></div>)}</div> : <p className="mt-4 text-sm text-[var(--text-muted)]">No admins or guards linked to this community yet.</p>}
                </div>
              ) : null}
              {previews.users.state === "unavailable" ? <p className="mt-4 text-sm text-[var(--text-muted)]">User preview unavailable.</p> : null}
            </SummaryCard>

            <SummaryCard title="Facilities summary" action={<CommunityFacilitiesDrawer facilities={previews.facilities.items} state={previews.facilities.state} triggerLabel="Manage facilities" />}>
              <div id="facilities-summary">
                {previews.facilities.state === "live" ? <div className="space-y-3">{previews.facilities.items.slice(0, 2).map((facility) => <div key={facility.id} className="rounded-[22px] border border-white/8 bg-white/4 px-4 py-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-white">{facility.name}</p><p className="mt-1 text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">{facility.opensAt} to {facility.closesAt}</p></div><Badge tone={facility.isActive ? "success" : "default"}>{facility.isActive ? "Active" : "Inactive"}</Badge></div><div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-[var(--text-muted)]"><span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1">{facility.slotMinutes} min</span><span className="rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-white">{facility.pricePerSlot}</span></div></div>)}</div> : <div><p className="text-base font-semibold text-white">{previews.facilities.state === "disabled" ? "Reservations disabled" : previews.facilities.state === "unavailable" ? "Preview unavailable" : "No facilities configured"}</p><p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">Add facilities to manage reservable areas and operating windows.</p></div>}
              </div>
            </SummaryCard>

            <SummaryCard title="Recent admin activity" action={<CommunityAdminActivityDrawer activities={adminActivity.items} triggerLabel="View full log" />}>
              {adminActivity.state === "live" ? <div className="space-y-3">{recentActivities.map((activity) => <div key={activity.id}><p className="text-sm font-semibold text-white">{activity.summary}</p><p className="mt-1 text-sm text-[var(--text-muted)]">{activity.actorName} · {activity.createdAt}</p></div>)}</div> : <div><p className="text-base font-semibold text-white">{adminActivity.state === "unavailable" ? "Activity preview unavailable" : "No admin activity yet"}</p><p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">Important administrative actions for this community will appear here.</p></div>}
            </SummaryCard>
          </div>
        </div>
        ...
        <aside className="space-y-6">
          <section className="rounded-[30px] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[0_18px_50px_rgba(2,6,23,0.2)]">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-violet-200">Quick actions</p>
            <div className="mt-4 space-y-3">{quickActions.map((action) => <QuickActionCard key={action.label} action={action} />)}</div>
          </section>
